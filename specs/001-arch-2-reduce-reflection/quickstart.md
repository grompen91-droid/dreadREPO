...
### ARCH-2 baseline (2026-05-30)

| Step | Result |
|------|--------|
| `gen-stubs.ps1` | pass |
| stub `dotnet build` Release | pass (0 errors) |
| `verify-dread.ps1` Tier 0 | pass (`ok: true`) |
| `Dread.ErrorReportJson.Tests` | pass (4/4) |
...
### Optional full-build + deploy smoke

1. Set `GameDir` to your game `REPO_Data/Managed` (see [contracts/build-profiles.md](./contracts/build-profiles.md)).
2. Set `BepInExDir` and `PluginDir` to your r2modman profile `BepInEx` paths.
3. Build with `-p:DeployToProfile=true`.
4. Launch R.E.P.O. from that profile.
5. Confirm BepInEx log shows Dread load; open menu (no init exception).
6. Optional: start a run, toggle debug overlay (F10) if enabled.
