...
| `player-tumble-compat-scan` | `Systems/PlayerTumbleCompat.cs` | `IsInTumble` field scan | per call | none | required | required | **reduce** | Same field cache pattern as controller compat |
| `repoconfig-slider-compat` | `Systems/RepoConfigSliderLabelCompat.cs` | Harmony + UI field/property access | on slider create | REPOConfig + MenuLib | required | required | **keep** | DBG-4 owns removal; optional mod types absent from stubs |
...
| `error-payload-game-state` | `Systems/ErrorReporting/ErrorReportPayloadCapture.cs` | `CaptureGameState` | log batch | error reporting on | optional | required | **keep** | `FindObjectsOfType` compile-time; try/catch for stub/mod mismatch |
