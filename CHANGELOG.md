...
### Fixed
- **Debug overlay (F10 crash):** overlay no longer throws `MissingMethodException: GUIContent.get_none()` when shown. IMGUI types (`GUI`, `GUIStyle`, `GUIContent`, `GUISkin`) now resolve from `UnityEngine.IMGUIModule` (where the game actually defines them) instead of being lumped into the `UnityEngine` stub assembly, and the box draw uses a cached empty `GUIContent` rather than the stub-only `GUIContent.none` property
